...
G4VPhysicalVolume* B5DetectorConstruction::Construct()
{
  ...
  // Steel Tube, note modified tube radius to allow for hollow PE
  auto nist = G4NistManager::Instance();
  G4Material* shape1_mat = nist->FindOrBuildMaterial("G4_Fe");
  G4ThreeVector pos1 = G4ThreeVector(0, 0*cm, -5*cm);
  G4double tuberadius = 8*cm;
  G4double tubethickness = 0.2*cm;
  G4double tubelength = 5*cm;
  ...
  //PE cylinder, note added thickness so tube is hollow
  G4Material* pe_mat = nist->FindOrBuildMaterial("G4_POLYETHYLENE");
  G4ThreeVector pos2 = G4ThreeVector(0, 0, -5*cm);
  G4double hcylinder = 5*cm;
  G4double pethickness = 7.5* cm;
  G4VSolid* cylinderSolid = new G4Tubs("PE", tuberadius-pethickness, tuberadius, hcylinder, 0*deg, 360*deg);
  ...
  //PE cylinder behind source to block
  G4ThreeVector blockpos = G4ThreeVector(0, 0, -6 * cm);
  G4double blocklen = 4 * cm;
  G4double blockthick = .5* cm;
  G4VSolid* blockSolid = new G4Tubs("PE", 0, blockthick, blocklen, 0 * deg, 360 * deg);
  G4LogicalVolume* logicblock = new G4LogicalVolume(blockSolid, pe_mat, "Blocker");
  new G4PVPlacement(0, blockpos, logicblock, "PE", worldLogical, false, 0, checkOverlaps);


  //PE Half-Sphere with hole surrounding target
  //Inner radius to match pipe radius
  //Thickness mean free path of 2MeV neutron in PE (~3cm)
  G4ThreeVector petargetspherepos = G4ThreeVector(0, 0, 0.1 * cm);
  G4double petargetsphereinnrad = 5 * cm;
  G4double petargetsphereoutrad = petargetsphereinnrad + 3 * cm;
  G4double petargetspheremintheta = 0 * deg;
  G4double petargetspheremaxtheta = 90* deg;
  G4Sphere* peTargetSphereShape =
	  new G4Sphere("peTargetSphere",
		  petargetsphereinnrad, petargetsphereoutrad,
		  0 * deg, 360 * deg,
		  petargetspheremintheta, petargetspheremaxtheta);
  G4LogicalVolume* logicpeTarget =
	  new G4LogicalVolume(peTargetSphereShape,
		  pe_mat,
		  "pe_target_sphere");
  new G4PVPlacement(0,
	  petargetspherepos,
  ...
  //Shape 3
  //postion ~1 in=~2.5 cm from source
  ...
  logicShape3 =
    new G4LogicalVolume(solidShape3,
                        sphere_mat,
                        "boron_sphere");
  G4ThreeVector pos3 = G4ThreeVector(0, 0, 2.5 * cm);;
  new G4PVPlacement(0,
                        pos3,
                        logicShape3,
                        "Shape3",
...
void B5DetectorConstruction::ConstructSDandField()
{
  ...
  auto borDetector = new G4MultiFunctionalDetector("Boron");
  sdManager->AddNewDetector(borDetector);

  G4VPrimitiveScorer* primitive;
  primitive = new G4PSEnergyDeposit("Edep");
  borDetector->RegisterPrimitive(primitive);

  primitive = new G4PSTrackLength("TrackLength");
  borDetector->RegisterPrimitive(primitive);

  SetSensitiveDetector("boron_sphere", borDetector);
}    
